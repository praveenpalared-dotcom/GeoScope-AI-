...
def filter_and_rename_keys(current_state, loaded_state_dict):
    ...
    # loaded_state_dict = {k: v for k, v in loaded_state_dict.items() if k.startswith("encoder_q")}
    renamed_state_dict = {}
    for current_key, (l_k, l_v) in zip(current_state.keys(), loaded_state_dict.items()):
        ...
        lk_p = l_k.split(".")[2:]  # encoder_q.4.0.bn1.weight -> 0.bn1.weight
        if ck_p == lk_p:
...
def build_seco(config):
    return SecoRN50(pretraining=True, **config.encoder.init_args.as_dict())

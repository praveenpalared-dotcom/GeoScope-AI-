...
class SwinTransformer(BaseCDEncoder):
    r"""Swin Transformer
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.return_down:
                ...
                ret_feat = no_down
            B, L, C = ret_feat.shape
            H = W = int(L**0.5)
            features[f"layer_{i + 1}"] = einops.rearrange(
                ret_feat, "b (h w) c -> b c h w", h=H, w=W
            )
    ...
    def get_parameters(self, pretraining: bool) -> list[dict]:
        ...
        model = self
        # no lr-scale since authors don't provide it
        scales = list(1**i for i in reversed(range(num_layers + 2)))
...
def build_gfm(config):
    encoder_args = config.encoder.init_args.as_dict()
    ws = encoder_args.pop("window_size")
    if ws is None:
        ws = 8
    model = SwinTransformer(
        ...
        drop_path_rate=0.1,
        pretraining=False,
        return_down=encoder_args.pop("return_down"),
        **encoder_args,
    )
...
def load_pretrained(config, model, logger):
    print(f">>>>>>>>>> Fine-tuned from {config.encoder.init_args.pt_path} ..........")
    ...
    #     checkpoint_model = {
    #         k: v for k, v in checkpoint_model.items() if "head" not in k
    #     }
    ...
    print(f">>>>>>>>>> loaded successfully '{config.encoder.init_args.pt_path}'")
            ...
            if nH1 != nH2:
                ...
                if L1 != L2:
                    ...
                    for i in range(nH1):
                        ...
                        # this changes (also adds z.T as per official transiton guide)
                        f_cubic_new = RegularGridInterpolator(
                            (x, y), z.T, method="cubic", bounds_error=False
                        )
                        # this is needed to match old behavior
                        xxnew, yynew = np.meshgrid(dx, dy, indexing="ij")

                        all_rel_pos_bias.append(
                            torch.Tensor(
                                f_cubic_new((xxnew, yynew)).T
                            )  # transition also adds transpose
                            .contiguous()
                            .view(-1, 1)

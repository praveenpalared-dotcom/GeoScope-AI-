...
class Attention(nn.Module):
    def __init__(
        ...
        self.full_attn_rel_pos_h = nn.Parameter(
            torch.zeros(rel_sp_dim, head_dim)
        )  # 2ws-1,C'
        self.full_attn_rel_pos_w = nn.Parameter(
            torch.zeros(rel_sp_dim, head_dim)
        )  # 2ws-1,C'
...
def calc_rel_pos_spatial(
    ...
    )  # B, H, qwh, qww, C'; qwh, kWh, C' -> B,H,qwh,qww,kwh
...
class RotatedVariedSizeWindowAttention(nn.Module):
    def __init__(
        ...
        if self.learnable:
            ...
            self.sampling_offsets = nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.Conv2d(
                    dim, self.num_heads * self.num_deform * 2, kernel_size=1, stride=1
                ),
            )
            ...
            self.sampling_angles = nn.Sequential(
                ...
                nn.LeakyReLU(),
                nn.Conv2d(
                    dim, self.num_heads * self.num_deform * 1, kernel_size=1, stride=1
                ),
            )
        ...
        if self.relative_pos_embedding:
            ...
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(
                    (window_size + attn_window_size - 1)
                    * (window_size + attn_window_size - 1),
                    num_heads,
                )
            )  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, H, W):
        ...
        image_reference = (
            torch.stack(torch.meshgrid(image_reference_w, image_reference_h), 0)
            .permute(0, 2, 1)
            .unsqueeze(0)
        )  # 1, 2, H, W

        # position of the window relative to the image center
        window_reference = nn.functional.avg_pool2d(
            image_reference, kernel_size=self.ws
        )  # 1,2, nh, nw
        image_reference = image_reference.reshape(
            1, 2, window_num_h, self.ws, window_num_w, self.ws
        )  # 1, 2, nh, ws, nw, ws
        assert window_num_h == window_reference.shape[-2]
        assert window_num_w == window_reference.shape[-1]

        window_reference = window_reference.reshape(
            1, 2, window_num_h, 1, window_num_w, 1
        )  # 1,2, nh,1, nw,1

        # coords of pixels in each window

        base_coords_h = (
            torch.arange(self.attn_ws).to(x.device)
            * 2
            * self.ws
            / self.attn_ws
            / (expand_h - 1)
        ...
        expanded_base_coords_h = base_coords_h.unsqueeze(dim=0).repeat(
            window_num_h, 1
        )  # ws -> 1,ws -> nh,ws
        assert expanded_base_coords_h.shape[0] == window_num_h
        assert expanded_base_coords_h.shape[1] == self.attn_ws
        expanded_base_coords_w = base_coords_w.unsqueeze(dim=0).repeat(
            window_num_w, 1
        )  # nw,ws
        ...
        window_coords = (
            torch.stack(
                torch.meshgrid(expanded_base_coords_w, expanded_base_coords_h), 0
            )
            .permute(0, 2, 1)
            .reshape(1, 2, window_num_h, self.attn_ws, window_num_w, self.attn_ws)
        )  # 1, 2, nh, ws, nw, ws
        ...
        if self.restart_regression:
            ...
            coords = base_coords.repeat(
                b * self.num_heads, 1, 1, 1, 1, 1
            )  # B*nH, 2, nh, ws, nw, ws
        if self.learnable:
            # offset factors
            ...
            sampling_offsets = sampling_offsets.reshape(
                num_predict_total, 2, window_num_h, window_num_w
            )
            sampling_offsets[:, 0, ...] = sampling_offsets[:, 0, ...] / (h // self.ws)
            sampling_offsets[:, 1, ...] = sampling_offsets[:, 1, ...] / (w // self.ws)

            # scale fators
            sampling_scales = self.sampling_scales(
                x
            )  # B, heads*2, h // window_size, w // window_size
            sampling_scales = sampling_scales.reshape(
                num_predict_total, 2, window_num_h, window_num_w
            )

            # rotate factor
            sampling_angle = self.sampling_angles(x)
            sampling_angle = sampling_angle.reshape(
                num_predict_total, 1, window_num_h, window_num_w
            )

            # first scale

            window_coords = window_coords * (
                sampling_scales[:, :, :, None, :, None] + 1
            )
            ...
            window_coords_r[:, 0, :, :, :, :] = -window_coords[
                :, 1, :, :, :, :
            ] * torch.sin(sampling_angle[:, 0, :, None, :, None]) + window_coords[
                :, 0, :, :, :, :
            ] * torch.cos(sampling_angle[:, 0, :, None, :, None])
            window_coords_r[:, 1, :, :, :, :] = window_coords[
                :, 1, :, :, :, :
            ] * torch.cos(sampling_angle[:, 0, :, None, :, None]) + window_coords[
                :, 0, :, :, :, :
            ] * torch.sin(sampling_angle[:, 0, :, None, :, None])
            ...
            coords = (
                window_reference
                + window_coords_r
                + sampling_offsets[:, :, :, None, :, None]
            )

        # final offset
        sample_coords = coords.permute(0, 2, 3, 4, 5, 1).reshape(
            num_predict_total,
            self.attn_ws * window_num_h,
            self.attn_ws * window_num_w,
            2,
        )
        sample_coords = sample_coords.type(x.dtype)

        qkv = (
            self.qkv(shortcut.permute(0, 2, 3, 1).reshape(b, -1, self.dim))
            .permute(0, 2, 1)
            .reshape(b, -1, h, w)
            .reshape(b, 3, self.num_heads, self.out_dim // self.num_heads, h, w)
            .transpose(1, 0)
            .reshape(3 * b * self.num_heads, self.out_dim // self.num_heads, h, w)
        )
        # if self.shift_size > 0:
        qkv = torch.nn.functional.pad(
            qkv, (padding_left, padding_right, padding_top, padding_down)
        ).reshape(
            3,
            b * self.num_heads,
        ...
        q, k, v = (
            ...
            qkv[1],
            qkv[2],
        )  # b*self.num_heads, self.out_dim // self.num_heads, H，W

        k_selected = F.grid_sample(
            k.reshape(
                num_predict_total,
                self.out_dim // self.num_heads // self.num_deform,
                h + padding_td,
                w + padding_lr,
            ),
            grid=sample_coords,
        ...
        if self.relative_pos_embedding:
            ...
            ].view(self.ws * self.ws, self.attn_ws * self.attn_ws, -1)  # Wh*Ww,Wh*Ww,nH
    ...
    def _reset_parameters(self):
        if self.learnable:
            nn.init.constant_(self.sampling_offsets[-1].weight, 0.0)
            nn.init.constant_(self.sampling_offsets[-1].bias, 0.0)
            nn.init.constant_(self.sampling_scales[-1].weight, 0.0)
            nn.init.constant_(self.sampling_scales[-1].bias, 0.0)
    ...
    def flops(
        ...
        h, w = (
            self.img_size[0] + self.shift_size + self.padding_bottom,
            self.img_size[1] + self.shift_size + self.padding_right,
        )
        ...
        if self.learnable:
            ...
            flops_sampling += (
                2 * (h / self.ws + w / self.ws) * self.num_heads * 2 * self.dim
            )
            # calculating the coords
            flops_sampling += (
                h / self.ws * self.attn_ws * w / self.ws * self.attn_ws * 2
            )
        # grid sampling attended features
        flops_sampling += (
            h / self.ws * self.attn_ws * w / self.ws * self.attn_ws * self.dim
        )
...
class RVSA_MTP(BaseCDEncoder):
            ...
            if full_attn_rel_pos_h:
                for k in list(state_dict.keys()):
                    if "full_attn_rel_pos_h" in k or "full_attn_rel_pos_w" in k:
                        ...
                        old_rel_pos = old_rel_pos.reshape(
                            1, 1, old_rel_sp_dim, old_head_dim
                        )

                        new_rel_pos = torch.nn.functional.interpolate(
                            old_rel_pos,
                            size=(new_rel_sp_dim, new_head_dim),
                            mode="bicubic",
                            align_corners=False,
                        )
    ...
    def forward_features(self, x):
        ...
        features = list(
            map(lambda x: x.permute(0, 2, 1).reshape(B, -1, Hp, Wp), features)
        )

        features = {f"layer_{i}": f for i, f in zip(indices, features)}
...
def vit_b_rvsa(config):
    ...
    backbone = RVSA_MTP(
        ...
        patch_size=16,
        drop_path_rate=0.3,
        out_indices=encoder_args["return_layers"],
        embed_dim=768,
        depth=12,
        num_heads=12,
        mlp_ratio=4,
        ...
        pretrained=encoder_args["pt_path"],
        pretraining=False,
        **encoder_args,
    )
...
def get_ft_params(params, module, prefix="", is_dcn_module=None):
    ...
    """
    parameter_groups = {}
    num_layers = 12 + 2  # vit-b
    layer_decay_rate = 0.9  # from MTP cfg
    print(
        "Build LayerDecayOptimizerConstructor %f - %d" % (layer_decay_rate, num_layers)
    )
